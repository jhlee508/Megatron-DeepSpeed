...
# Load the correct conda environment for each node
if [[ $(hostname) == em* ]]; then
  conda activate torch-amd-megatron-deepspeed
elif [[ $(hostname) =~ ^v0[0-7]$ ]]; then
  conda activate torch-nvidia-megatron-deepspeed
else
  conda activate torch-nvidia-megatron-deepspeed
fi
...
### Data
data_path="data/mega-ds-gpt2-oscar_text_document"
...
DISTRIBUTED_ARGS="
    ...
    --rdzv_id 1234 
    --rdzv_backend c10d 
    --rdzv_endpoint $MASTER_NODE
"
export OMP_NUM_THREADS=$(($(grep -c processor /proc/cpuinfo) / $NPERNODES))
...
torchrun $DISTRIBUTED_ARGS \
    ...
    ${data_options} \
    ${deepspeed_options} \
    |& tee ${log_path}/${jobname}_${host}_${current_time}.log